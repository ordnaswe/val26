...
def one_cycle(args):
    ...
    try:
        ...
        if _dj.exists():
            ...
            _pcmd = [sys.executable, str(HERE / "gor_mandat.py"),
                     "--data", str(_dj), "--valkrets", args.valkrets,
                     "--mandat", str(HERE / "data" / "mandat.csv"), "--out", str(_mandat),
                     "--distrikt-fallback", str(HERE / "data" / "rd_baseline.csv")]
            if _rd and Path(_rd).exists():
                _pcmd += ["--distrikt", _rd]
